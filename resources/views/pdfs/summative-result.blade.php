...
<div class="pdf-wrapper">
        ...
        <table>
            ...
            @foreach($assessments as $assessment)
                <tr>
                    ...
                    <td>{{ checkSummetiveCriteria(round($point, 0)) }}</td>
                    <td>{{ checkSummetiveCriteria(round($point, 0), true) }}</td>
                </tr>
            @endforeach
            </tbody>
            <tfoot>
            <tr>
                ...
                @endphp
                <th>Average</th>
                <th style="text-align: right">{{ round($average, 0) }}%</th>
                <th>{{ checkSummetiveCriteria(round($average, 0)) }}</th>
                <th>{{ checkSummetiveCriteria(round($average, 0), true) }}</th>
            </tr>
            </tfoot>
        </table>
    </div>
    <div class="general-text">Learner General Average:  {{ checkSummetiveCriteria(round($average, 0)) }}</div>
    <p>{{ __('Teacher Comment:') }} {{ checkSummetiveCriteria(round($average, 0), false, rand(1, 10)) }}</p>
    <div class="term-dates-table">
        <table>
            ...
            <tr>
                <th>
                    <p>{{ __('Signature_____________________') }}</p>
                    <!--<div class="border"></div>-->
                    <p>Class Teacher</p>
                </th>
                <th>
                    <p>{{ __('Signature____________________') }}</p>
                    <!--<div class="border"></div>-->
                    <p>Principal</p>
                </th>
            </tr>
